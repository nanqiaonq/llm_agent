...
export function KnowledgeGraph({ onNext, onPrevious, selectedDocument }: KnowledgeGraphProps) {
  ...
  // 视图模式：关系链视图 vs 完整网络图
  const [viewMode, setViewMode] = useState<'chains' | 'network'>('chains');
  ...
  const mockRelationships = doc?.extracted?.relationships || [];
  const mockEntities = doc?.extracted?.entities || [];

  // 加载粘贴文本的提取结果
  useEffect(() => {
    if (isPastedText) {
      const stored = sessionStorage.getItem('extractedItems');
      console.log('[KnowledgeGraph] Loading extractedItems from sessionStorage:', stored);
      if (stored) {
        try {
          const items: ExtractedItem[] = JSON.parse(stored);
          console.log('[KnowledgeGraph] Parsed items:', items);
  ...
  }, [isPastedText, selectedDocument]); // 添加 selectedDocument 作为依赖项，确保每次切换页面都重新加载
  ...
  const relationChains = useMemo((): RelationChain[] => {
    ...
    extractedItems.forEach(item => {
      ...
      if (mechanismGroup && !seenMechanisms.has(mechanismGroup)) {
        seenMechanisms.add(mechanismGroup);
        const nodes = mechanismGroup.split('-').filter((n: string) => n.trim());
        if (nodes.length >= 2) {
          chains.push({
            nodes,
            mechanismGroup,
    ...
    console.log('[KnowledgeGraph] Extracted relation chains:', chains);
    return chains;
  }, [extractedItems]);
  ...
  const filteredNodes = useMemo(() => {
    ...
    return result;
  }, [nodes, selectedTypes, showAllTypes, viewMode]);
  ...
  useEffect(() => {
    ...
    let entities: Array<{ value: string; type: string; role: string; highlight: string; attributes?: Record<string, string> }>;

    if (isPastedText && extractedItems.length > 0) {
      relationships = generateRelationshipsFromItems(extractedItems);
      entities = extractedItems.map(item => ({
        value: item.value,
        type: item.type,
        role: item.role,
  ...
  }, [selectedDocument, isPastedText, extractedItems, doc]);
  ...
  const getSelectedNodeData = () => {
    ...
    if (isPastedText) {
      ...
      return mockEntities.find(e => e.value === selectedNode);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <TeachingBanner
        title="这一步在做什么？"
        description="AI 从提取的实体中发现知识关系，构建知识图谱。每个关系链展示实体之间的作用机制和关联路径。"
        tips={[
          '每张卡片代表一条完整的知识关系链',
          '箭头表示关系的方向，从起点到终点',
          '点击卡片可以查看详细信息',
        ]}
        comparison="这就像绘制一张知识地图，让复杂的关系变得清晰可见。"
      />
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-200 p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-indigo-500">
                {viewMode === 'simplified' ? '只显示核心实体' : '显示全部节点'}
              </span>
            </div>
              ...
              <div>
                <div className="text-[10px] text-gray-500 mb-2 flex items-center gap-1">
                  ...
                  关联实体 ({selectedConnections.filter(c => {
                    ...
                    return viewMode === 'full' || otherNodeData?.isCore;
                  }).length})
                </div>
