...
@router.get("/cache/stats", tags=["系统"])
async def get_cache_stats(extractor: Extractor = Depends(get_extractor)):
    """获取缓存统计"""
    return extractor.cache.stats()


@router.delete("/cache", tags=["系统"])
async def clear_cache(extractor: Extractor = Depends(get_extractor)):
    """清空缓存"""
